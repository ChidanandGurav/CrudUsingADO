...
namespace CrudUsingADO.Models
{
    public class ProductDAL
    {
        ...
        {
            Product pro = new Product();
            string qry = "select p.*,c.cname from Product p inner join Category c on c.cid=p.cid where p.pid=@id and p.isActive=1";
            cmd = new SqlCommand(qry, con);
        ...
        public int AddProduct(Product pro)
        {
            pro.IsActive = 1;
            int result = 0;
            string qry = "insert into Product values(@name,@price,@company,@cid,@imageurl, @isActive)";
            cmd = new SqlCommand(qry, con);
        ...
        public int UpdateProduct(Product pro)
        {
            pro.IsActive = 1;
            int result = 0;
            string qry = "update Product set name=@name,price=@price,company=@company,cid=@cid,imageurl=@imageurl where pid=@id";
            cmd = new SqlCommand(qry, con);
            cmd.Parameters.AddWithValue("@name", pro.Name);
            cmd.Parameters.AddWithValue("@price", pro.Price);
            //cmd.Parameters.AddWithValue("@company", pro.Company);
        ...
        {
            int result = 0;
            string qry = "update Product set isActive=0 where pid=@id";
            cmd = new SqlCommand(qry, con);
            cmd.Parameters.AddWithValue("@id", id);
            con.Open();
            result = cmd.ExecuteNonQuery();
